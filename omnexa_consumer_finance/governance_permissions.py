...
def policy_query_conditions(user=None):
	...
	return f"(`tabConsumer Finance Policy Version`.maker = {esc} or `tabConsumer Finance Policy Version`.checker = {esc} or `tabConsumer Finance Policy Version`.rejector = {esc})"
...
def snapshot_query_conditions(user=None):
	...
	if _is_privileged(user):
		return ""
	esc = frappe.db.escape(user)
	return f"`tabConsumer Finance Audit Snapshot`.actor = {esc}"
