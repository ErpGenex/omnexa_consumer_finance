...
def execute(filters=None):
	...
	pol_total = frappe.db.count("Consumer Finance Policy Version")
	pending = frappe.db.count("Consumer Finance Policy Version", {{"status": "PENDING_APPROVAL"}})
	approved = frappe.db.count("Consumer Finance Policy Version", {{"status": "APPROVED"}})
	rejected = frappe.db.count("Consumer Finance Policy Version", {{"status": "REJECTED"}})
	snaps = frappe.db.count("Consumer Finance Audit Snapshot")
